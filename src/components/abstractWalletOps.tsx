  ...
  const wallet_connect_banner = () => {
    if (isConnected) {
      return (
        <>
        <Alert>
          <AlertTitle>Your account Wallet is successfully connected</AlertTitle>
          <AlertDescription> with link:  <a href={`${process.env.NEXT_PUBLIC_SCAN_URL}address/${wallet.getAddress()}`} target="_blank">
            </a>
          </AlertDescription>
...
export function useWalletAuth() {
  ...
  const [isConnected, setIsConnected] = useState(false);
   const [connectionStatus, checkConnectionStatus] = useState<string | null>(null);
