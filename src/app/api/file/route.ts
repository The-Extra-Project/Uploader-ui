...
export async function POST(request: Request) {
 ...
 // if (!env.S3_BUCKET_NAME) return new Response(null, { status: 500 });
  const { filename, filepath, error } = await request.json();
  if (error) {
    return NextResponse.json({ code: "404" });
  }

  const sql = db.from("admin");
  const stockage = db.storage.from("uploader_ui")
  ...
  try {
    ...
    let signedUrl = stockage.createSignedUploadUrl(filepath)
    alert("the signedUrl is created here " + signedUrl)
